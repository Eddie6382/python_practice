from vpython import *
size, m_o, m_c, k_bond = 15E-12, 16.0/6E23, 12.0/6E23, 18600.0    # These numbers are all made up
d = 5*size
dt = 1E-16
class CO_molecule:
    ...
    def com_v(self):                  # return velocity of center of mass
        return ( self.O.m*self.O.v + self.C.m*self.C.v )/( self.O.m + self.C.m )
    ...
    def v_K(self):                    # return kinetic energy of the vibration motion
        v_K_O = 1/2*self.O.m*( mag(proj(self.O.v - self.com_v(), self.bond.axis)) )**2
        v_K_C = 1/2*self.C.m*( mag(proj(self.C.v - self.com_v(), self.bond.axis)) )**2
        return v_K_O + v_K_C
    def r_K(self):                    # return kinetic energy of the rotational motion
        r_K_O = 1/2*self.O.m*( mag(self.O.v - self.com_v())*sin(diff_angle(self.O.v-self.com_v(), self.bond.axis)) )**2
        r_K_C = 1/2*self.C.m*( mag(self.C.v - self.com_v())*sin(diff_angle(self.C.v-self.com_v(), self.bond.axis)) )**2
        return r_K_O + r_K_C
